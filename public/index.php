...
?>
<div class="card-verify" style="max-width: 860px;">
  <div style="text-align: center; margin-bottom: 26px;">
    <div style="display: inline-flex; align-items: center; justify-content: center; padding: 18px; background-color: rgba(59, 130, 246, 0.08); border-radius: 20px; margin-bottom: 16px; color: #3b82f6; box-shadow: 0 10px 20px -5px rgba(59, 130, 246, 0.2);">
      <i class="ki-filled ki-shield-search" style="display: block; font-size: 2.5rem; line-height: 1;"></i>
    </div>
  ...
  <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; margin-bottom: 22px;">
    <span class="history-badge <?= $Minhnhatdev_remaining > 0 ? 'status-valid' : 'status-invalid' ?>">
      <i class="ki-filled ki-status"></i> Còn lại hôm nay: <strong><?= $Minhnhatdev_remaining ?></strong> / <?= (int)$Minhnhatdev_user['daily_quota'] ?> lượt
    </span>
    <span class="history-badge">
      <i class="ki-filled ki-double-check"></i> Đã dùng: <strong><?= $Minhnhatdev_used ?></strong> lượt
    </span>
  </div>
  ...
  <form id="MinhnhatdevCheckForm" onsubmit="return MinhnhatdevSubmitCheck(event)">
    ...
    <div style="margin-bottom: 18px;">
      ...
      <div class="input-group-responsive">
        ...
        <button id="MinhnhatdevBtn" type="submit" class="btn-submit" <?= $Minhnhatdev_remaining <= 0 ? 'disabled style="opacity:.5; cursor:not-allowed;"' : '' ?>>
          <i class="ki-filled ki-magnifier"></i> <span class="btn-text">Check Ngay</span>
        </button>
      </div>
    </div>
    ...
    <div class="Minhnhatdev-alert Minhnhatdev-alert-err">
      <i class="ki-filled ki-information-2"></i> Bạn đã dùng hết <?= (int)$Minhnhatdev_user['daily_quota'] ?> lượt check hôm nay. Quay lại vào ngày mai hoặc liên hệ admin để nâng hạn mức.
    </div>
    <?php endif; ?>
  ...
  <div style="margin-top: 25px; padding: 18px; border-radius: 16px; background-color: #f8fafc; border: 1px solid rgba(226, 232, 240, 0.8); box-sizing: border-box;">
    <h6 style="font-weight: 700; color: #1e293b; font-size: 13px; margin: 0 0 8px 0; display: flex; align-items: center; gap: 6px;">
      <i class="ki-filled ki-information-2 text-primary" style="font-size: 16px;"></i> Lưu ý quan trọng
    </h6>
...
function MinhnhatdevRenderResult(d) {
  ...
  if (typeof d.remaining !== 'undefined' && quota) {
    quota.innerHTML = '<i class="ki-filled ki-status"></i> Còn lại hôm nay: <strong>' + d.remaining + '</strong> / ' + d.quota + ' lượt';
  ...
  if (d.status === 'HIT') {
    html = '<div style="padding: 16px; border-radius: 16px; background: rgba(16,185,129,.06); border: 1px solid rgba(16,185,129,.3);">'
      + '<div style="display:flex; align-items:center; gap:8px; margin-bottom:12px; color:#047857; font-weight:800; font-size:15px;">'
      + '<i class="ki-filled ki-double-check" style="font-size:20px;"></i> HIT - Đăng nhập thành công! '
      ...
      + '<button type="button" onclick="MinhnhatdevCopyLine()" class="btn-submit" style="margin-top:12px; height:36px; font-size:12px;"><i class="ki-filled ki-copy"></i> Copy kết quả</button>'
      + '</div>';
  } else {
    ...
    html = '<div style="padding: 16px; border-radius: 16px; background: rgba(239,68,68,.06); border: 1px solid rgba(239,68,68,.3);">'
      ...
      + '<i class="ki-filled ki-cross-circle" style="font-size:18px;"></i> ' + MinhnhatdevEsc(label) + '</div>'
